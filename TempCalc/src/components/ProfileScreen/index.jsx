...
const HomeScreen = () => {
  ...
  const [quantidadeArroz, setQuantidadeArroz] = useState('');
  const [quantidadeFeijaopreto, setQuantidadeFeijaopreto] = useState('');
  const [quantidadeCalabresa, setQuantidadeCalabresa] = useState('');
  const [quantidadeBacon, setQuantidadeBacon] = useState('');
  const [quantidadeAgisal, setQuantidadeAgisal] = useState('');
  const [quantidadeLatamilho, setQuantidadeLatamilho] = useState('');
  const [quantidadeCheirov, setQuantidadeCheirov] = useState('');
  const [quantidadeArrozpar, setQuantidadeArrozpar] = useState('');
  const [quantidadeColve, setQuantidadeColve] = useState('');
  const [quantidadeCarioca, setQuatidadeCarioca] = useState('');
  const [quantidadeCenoura, setQuantidadeCenoura] = useState('');
  const [quantidadeErvilha, setQuantidadeErvilha] = useState('');
  const [quantidadeBatata, setQuantidadeBatata] = useState('');
  ...
  const calcularQuantidades = (peso, option) => {
    if (option === 'Arroz') {
      const quantidadeAgisal = (24 * parseFloat(peso)).toFixed(0);
      const quantidadeAlho = (9 * parseFloat(peso)).toFixed(0);
      const quantidadeLouro = (65 * parseFloat(peso)).toFixed(0); 
      const quantidadeOleo = (116 * parseFloat(peso)).toFixed(0);

      setQuantidadeAlho(quantidadeAlho);
      setQuantidadeAgisal(quantidadeAgisal);
      ...
      <View style={styles.containnerSelect}>
        ...
        <View style={styles.carne}>
          <TouchableOpacity
            style={[
              styles.tipoCarne,
              selectedOption === 'Arroz' && styles.selectedOption,
            ]}
            onPress={() => handleOptionSelect('Arroz')}
          >
            <Text style={{ fontSize: 20, marginVertical: 10 }}>Arroz</Text>
          </TouchableOpacity>
        ...
        {selectedOption !== '' && (
          <View style={styles.temperoContainer}>
            ...
            {selectedOption === 'Arroz' && (
              <>
                <Text style={styles.temperoText}>
                  Agisal: {quantidadeColorau}g
                </Text>
                <Text style={styles.temperoText}>
                  Louro: {quantidadeColorau}g
                </Text>
                <Text style={styles.temperoText}>
                  oleo: {quantidadeColorau}g
                </Text>
                <Text style={styles.temperoText}>
